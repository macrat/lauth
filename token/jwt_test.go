...
func TestCodeToken(t *testing.T) {
	jwtManager, err := testutil.MakeJWTManager()
	if err != nil {
...
func TestAccessToken(t *testing.T) {
	...
	accessToken, err := jwtManager.CreateAccessToken(issuer, "someone", "openid profile", time.Now(), 10*time.Minute)
	if err != nil {
		t.Fatalf("failed to generate token: %s", err)
	}

	claims, err := jwtManager.ParseAccessToken(accessToken)
	if err != nil {
	...
	code, _ := jwtManager.ParseCode(accessToken)
...
func TestJWTManager_JWKs(t *testing.T) {
	...
	manager, err := token.NewJWTManager(&pri)
	if err != nil {
